...
do
	...
	CUDA_VISIBLE_DEVICES=$GPU $ProtEvol -p $ProjectName -s $Seed -m 0.0001 -N $PopSize -t $SimTime -C $CompScale -T $TargetStruct -i $InitSeq -mu_insertion 0.0 -mu_deletion 0.0 -mu_duplication 0.0 -mu_ablation 0.0 -mu_reversion 0.0 -mu_transposition 0.0 > /linuxhome/tmp/sam/protevol/${ProjectName}.log
done
